...
Secrets Manager implements edit sessions every time a user needs to edit secrets inside a secret group to guarantee service of a consistent set of secrets. Secrets Manager ensures that operations are not interrupted while you are modifying secrets.
...
After you complete your changes, the secret group is updated with all the updated secrets that Secrets Manager will serve to all redeployed applications after that point.

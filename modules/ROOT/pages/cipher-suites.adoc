...
When you configure a TLS Context, default ciphers are applied with the TLS context based on the TLS version you selected. If you don't use the defaults, you can select ciphers that match the configured keystore to ensure that TLS can set up a connection: 

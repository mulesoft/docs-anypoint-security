...
If a lot of complex data processing must take place before traffic gets to the tokenization service, it makes sense to set up a Mule app to which the edge can route traffic. In this scenario, the Mule app processes the data and sends the traffic to the tokenization service. 

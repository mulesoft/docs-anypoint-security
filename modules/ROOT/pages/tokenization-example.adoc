...
. In Anypoint Platform, go to Exchange, and search for "AMC Tokenization."
. Download the `AMC Tokenizer` zip file.
. Go to *Design Center* and select *Create > API Specification*, and name it "AMC Tokenizer". +
...
. Delete the `amc.tokenizer.rml` and keep the `AnypointSecurityToken.raml` you uploaded.

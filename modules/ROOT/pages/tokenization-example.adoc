...
. xref:1.2@runtime-fabric::install-create-rtf-arm.adoc[Install Runtime Fabric]. Anypoint Runtime Fabric is a container service that automates the deployment and orchestration of Mule applications and API gateways.
. To allow Mule applications or API gateways to listen on inbound connections, enable inbound traffic, you must xref:1.2@runtime-fabric::enable-inbound-traffic.adoc[Enable inbound traffic] on Runtime Fabric.
...
. Deploy the app to Runtime Fabric. See xref:1.2@runtime-fabric::deploy-index.adoc[Deploy Applications and API Proxies to Runtime Fabric].

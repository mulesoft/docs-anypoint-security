...
[NOTE]
Because the tokenization service accepts only TLS traffic, you must configure TLS for the Mule app and enable the LastMile security. 
...
The API Gateway proxies requests to the tokenization service, provides authorization and authentication, and provides JSON protection for the tokenization service.
...
[NOTE]
Tokenization involves sensitive data, and therefore, works only with TLS. You must download the automatically generated API gateway Mule app and manually add TLS to the API gateway Mule app connectors before the tokenization service will work. +
...
. Go to Anypoint Studio, and import the API gateway Mule app.
+

...
You can apply a tokenization policy to the API gateway capability of Mule 4 to serve as the intermediary between the Runtime Fabric inbound load balancer and the Mule app. 
...
When extra data processing is required, you can configure the tokenization service so that the Mule app processes the data and sends the traffic to the tokenization service. 

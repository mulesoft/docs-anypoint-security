...
+
[NOTE]
If a body is larger than 1 MB, the scan is skipped. If request body checking is enabled, the HTTP Limits policy *Maximum Message Size* should be set to a value no larger than required to prevent attackers from abusing request body checking, and exhausting resources.
...
[IMPORTANT]
====
If a body is larger than 1 MB, the body scan is skipped. 
====
+

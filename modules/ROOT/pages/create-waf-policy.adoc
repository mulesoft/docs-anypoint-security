...
+
[NOTE]
The *Disable body scanner* option must be checked if any valid responses will exceed 512 KB. If request body checking is enabled, the CAP policy “Maximum Message Size” should be set to a value no larger than required to prevent attackers from abusing request body checking, and exhausting the resources.
. Click *Save*.

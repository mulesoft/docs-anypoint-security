...
image::security-policy-list.png[Blue Create Policy button located above the table of security policies which lists the name, type, and action.]
...
+
image::waf-rule-set.png[Rule ID range is displayed for Scanner detection as a pop-up message.]
// not sure if this one is necessary; it adds something for a seeing user I guess but the alt text wouldn't really add anything new 
+

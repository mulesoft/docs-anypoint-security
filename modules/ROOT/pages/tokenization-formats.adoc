...
|*Ignore characters*
|Used by the masking domain to specify which characters are not masked. For example, if `Ignore characters` is set to `-` and the masking character is set to `\#`, masking applied to the number `123-45-6789` returns `\##\##-\##\#-####`.
|Masking

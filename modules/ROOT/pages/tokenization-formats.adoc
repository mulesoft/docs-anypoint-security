...
A decimal digit is be mapped into the Latin alphabet character range.
...
|*Ignore characters*
|Used by the masking domain to specify which characters are not masked. For example, if `Ignore characters` is set to `-` and the masking character is set to `#` then masking applied to the number `123-45-6789` would return `###-###-####`.
|Masking

|*Masking character*
|Used by the masking domain to specify the character that is used to hide sensitive data. For example, the `#` character.
|Masking
|===

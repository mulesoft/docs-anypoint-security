...
* If a client device supports AES-dedicated CPU instructions, the client should use the AES-GCM ciphers. 
* If the client does not support AES dedicated instructions (some mobile and IoT devices), the client will negotiate to use `CHACHA20-POLY1305`. It takes two (client and server) to do the CHACHA.

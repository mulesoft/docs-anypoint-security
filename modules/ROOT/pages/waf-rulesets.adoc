...
See xref:runtime-fabric::enable-inbound-traffic#logs[the Runtime Fabric documentation].
...
The Runtime Fabric policy summary includes the WAF summary. A WAF summary log message is generated every five minutes. See xref:runtime-fabric::configure-log-forwarding[Configure Log Forwarding on Runtime Fabric]. The contents in the raw log message include WAF and DoS summary statistics.

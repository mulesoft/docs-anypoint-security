...
The Runtime Fabric policy summary includes the WAF summary. A WAF summary log message is generated every one minute. See xref:1.2@runtime-fabric::configure-log-forwarding[Configure Log Forwarding on Runtime Fabric]. The contents in the raw log message include WAF and DoS summary statistics.

...
The Runtime Fabric policy summary includes the WAF summary. A WAF summary log message is generated every minute. See xref:1.13@runtime-fabric::runtime-fabric-logs.adoc[View and Configure Logging in Runtime Fabric]. The contents in the raw log message include WAF and DoS summary statistics.

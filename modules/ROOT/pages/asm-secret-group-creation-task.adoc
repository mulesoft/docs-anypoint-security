...
Using Secrets Manager you can create a secrets group and add secret types to it.
...
After creating your secret group, it appears in the *Secret Groups* list view. Edit the secret group to add the necessary secret types, for example adding a truststore or a keystore.
...
Edit a secret group to add secret types such as Keystore, Truststore, and Certificates.

. In the *Secret Groups* list view, select the secret group you want to edit, and click *Edit*. 
. Make your changes as required, then click *Finish.*

The secret group is updated with the changes, and the secrets manager provides the updated secrets to all redeployed applications after this point. 

...
include::partial$secret-group-limits.adoc[tag=secret-group-limit-note]
...
+
NOTE: If you select this option, secrets can be downloaded over public networks.  
...
[NOTE]
Some PKCS12 files generated with newer OpenSSL defaults (AES/SHA-256) might not upload and can return `RB-1003: invalid content`.
If upload fails, generate the PKCS12 file with Java `keytool` or use OpenSSL legacy PKCS12 settings for broader compatibility.
This approach helps ensure that Secrets Manager can parse the keystore successfully.

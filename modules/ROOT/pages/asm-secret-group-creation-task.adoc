...
Add a PEM or JKS certificate to the secret group. The certificate is a single certificate placed in the CA path when you created the keystore.  
...
The CA path contains the other certificates that are related to the single certificate file and are used by the other end of the SSL connection to decide which certificates will be accepted by the system using the keystore. This can also be used by the other system to fill in missing certificates.   
** *Override Expiration Date* (Optional) - Select the date to override the default expiration date of the certificate.

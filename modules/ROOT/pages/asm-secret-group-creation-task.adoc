...
The secret group appears in the *Secret Groups* list view. Now you can add the necessary secret types.
...
** *Alias* - The alias referenced by the Java program.
...
A truststore includes only public certificates of trusted servers. 

. In the *Secret Groups* list view, select the secret group to which to add a truststore. 
...
// _COMBAK: What type of certificate to upload?
...
== Add a Certificate Revocation List Distributor Configuration

Add a list of certificates that were revoked by certificate authorities before their expiration dates.

. In the *Secret Groups* list view, select the secret group to which to add a Certificate List Distributor (CRL) configuration. 
. Select *CRL Distributor Config* in the menu on the left, and click *Add CRL*. 

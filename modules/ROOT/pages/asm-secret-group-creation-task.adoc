...
The secret group appears in the *Secret Groups* list view. You can now add the necessary secret types, for example, a keystore or a truststore. 
...
You can configure a server or client to specify what to pin to improve the security of services that rely on SSL certificates. 
...
To include a certificate-key pair in a TLS context to encrypt inbound traffic to Runtime Fabric or Mule 4 based proxies, you must enable the TLS context.  

//tag::intro1[]
Using encryption, Transport Layer Security (TLS) protects communication between clients, API Instances, and upstream services. After you xref:asm-secret-group-creation-task.adoc[create a secret group] and xref:asm-secret-group-creation-task.adoc#adding-a-keystore[add a Privacy-Enhanced Mail (PEM) type keystore], you can add a TLS context and apply the TLS context to API Instances in API Manager to encrypt their inbound and outbound traffic.
...
* *Outbound Settings* +
Optionally, select *Skip server certificate validation* to skip the validation of the outbound server and not support mTLS for outbound traffic.
...
[[mTLS-context]]
== mTLS Context
Flex Gateway supports mutual authentication TLS (mTLS) for the inbound and outbound direction.

In mTLS both the server and the client validates the others TLS certification. In normal TLS only the client validates the server's certificate. For Flex Gateway outbound traffic, the API upstream is the server and Flex Gateway is the client. For Flex Gateway inbound traffic, Flex Gateway is the server and the client is the client making requests to Flex Gateway. 

Enable mTLS using the following UI:

* *Validate Client certificate* 
+
For inbound traffic, because the server is Flex Gateway you can choose to validate the clients certificate enabling inbound mTLS. However, you cannot require the client to validate Flex Gateway's certificate.

* *Skip server certificate validation* 
+
For outbound traffic, because the client is Flex Gateway you can choose to skip server certification validation disabling outbound mTLS. However, you cannot require the upstream to validate Flex Gateway's certificate. 

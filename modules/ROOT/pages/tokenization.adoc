...
In this example, a tokenization format is created with the credit card data domain and assigned to the tokenization service. A tokenization policy is applied to an app deployed to Runtime Fabric. 

When a payload that contains credit card numbers in the payload is sent, the request is redirected to the application that has the tokenization policy applied. 

Since the tokenization policy is configured to tokenize the credit card information in the payload, it extracts all the tokens from the payload. 

A connection is established with the tokenization service, which receives the tokens, applies the data transformation, and returns the tokenized data to the tokenization policy. 

The tokenization policy replaces the credit card number with tokenized values and the request is sent on to the app. The app processes the request and returns the result. 

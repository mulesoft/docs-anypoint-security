...
In the above example, using a masking format that preserves the last four characters option would return the token: `\####\##########--4567`. The identity of a masked token cannot be returned via a detokenization call to the tokenization service.

[WARNING]
====
If you configure a policy in API Manager to detokenize and you select a masking format, the API gateway returnsd a `400 Bad Request` response to the API. The API gateway returns this response because the masking format only masks data based on the original value. It cannot unmask data and return original value.
====
...
* Meet all the prerequisites and requirements for running Anypoint Runtime Fabric. +

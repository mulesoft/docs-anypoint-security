...
The way you configure and use the Tokenization service varies according to your implementation. For example, you can add the Tokenization service policy as one of the provided policies for the API Gateway, or, where extra data processing is required, you can expose the Tokenization endpoint through a Mule app.

See xref:apply-tokenization-policy-to-api.adoc[Apply a Tokenization Policy to the API Gateway] and xref:tokenization-example.adoc[Use the Tokenization Service on the Edge]
